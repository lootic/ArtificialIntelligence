(define (domain shakey)
	...
	(:action grip
		...
		:precondition 	(and (GRIP ?grip) (not (used ?grip)) (in-room ?room) (is-lit ?room) (item-location ?item ?room))
		:effect (and (used ?grip) (gripped-item ?item ?grip) 
				(not (item-location ?item ?room))))
	
	(:action drop
		:parameters (?room ?item ?grip)
		:precondition (and (in-room ?room) (gripped-item ?item ?grip))
		:effect (and (not (used ?grip)) (not(gripped-item ?item ?grip)) 
				(item-location ?item ?room)))
)
